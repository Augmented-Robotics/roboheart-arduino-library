...
#define DEBUG_BLE(x) {if (_debug != NULL) {_debug->print("[BLE_DEBUG] "); _debug->print(x);}}
#define DEBUG_LN_BLE(x) {if (_debug != NULL) {_debug->print("[BLE_DEBUG] "); _debug->println(x);}}
#define DEBUG(x) {if (_debug != NULL) {_debug->print(x);}}
#define DEBUG_LN(x) {if (_debug != NULL) {_debug->println(x);}}


static BLE_UUID_Config_t uuids = {  RH_APP_SERVICE_UUID, 
                                    RH_APP_CHARACTERISTIC_UUID1, 
...
InterfaceBLE::InterfaceBLE(Stream& debug)
    : _debug(&debug), configured(false)
{}

InterfaceBLE::InterfaceBLE()
    : _debug(NULL), configured(false)
{}
...
class CharacteristicCallbacks: public BLECharacteristicCallbacks {
    void onWrite(BLECharacteristic *pCharacteristic) {
        InterfaceBLE::charOnWriteInvCallback(pCharacteristic->getUUID().toString(),pCharacteristic->getValue());
    }
};
...
void InterfaceBLE::configure(uint8_t* package, uint8_t package_size, BLE_UUID_Config_t* config_uuids){
...
void InterfaceBLE::setCharacteristicsCallbacks(void (*char1write) (std::string), void (*char2write) (std::string), void (*char3write) (std::string)){
    char1writeCallback = char1write;
    char2writeCallback = char2write;
    char3writeCallback = char3write;

}
void InterfaceBLE::setServerCallbacks(void (*onConnect) (void), void (*onDisconnect) (void)){
