...
#define DEBUG_STEPPERMOTOR(x) {if (_debug != NULL) {_debug->print("[STEPPER_MOTOR_DEBUG] "); _debug->print(x);}}
#define DEBUG_LN_STEPPERMOTOR(x) {if (_debug != NULL) {_debug->print("[STEPPER_MOTOR_DEBUG] "); _debug->println(x);}}
...
RoboHeartStepperMotor::RoboHeartStepperMotor(Stream& debug)
: _debug(&debug)
{
...
void RoboHeartStepperMotor::begin(RoboHeartDRV8836* motor0, RoboHeartDRV8836* motor1) {
    _motor0 = motor0;
    _motor1 = motor1;
}

void RoboHeartStepperMotor::executeHalfStep(int cmd, StepperDirection_t direction) {
